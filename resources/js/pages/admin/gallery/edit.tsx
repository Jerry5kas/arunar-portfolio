...
export default function GalleryEdit({ galleryImage }: Props) {
    const { data, setData, processing, errors } = useForm({
        title: galleryImage.title || '',
        slug: galleryImage.slug || '',
        ...
        is_published: galleryImage.is_published || false,
        published_at: galleryImage.published_at ? galleryImage.published_at.substring(0, 16) : '',
        meta_title: galleryImage.meta_title || '',
        meta_description: galleryImage.meta_description || '',
        meta_keywords: galleryImage.meta_keywords || '',
        og_image: null as File | null,
        og_title: galleryImage.og_title || '',
        og_description: galleryImage.og_description || '',
    });

    const imageRef = useRef<HTMLInputElement>(null);
    const ogImageRef = useRef<HTMLInputElement>(null);
    const [imagePreview, setImagePreview] = useState<string>(
        galleryImage.image || galleryImage.image_url || ''
    );
    ...
    const submit: FormEventHandler = (e) => {
        ...
        if (hasFiles) {
            ...
            router.post(`/admin/gallery/${galleryImage.id}`, formData, {
                preserveScroll: true,
                onSuccess: () => {
            ...
            router.put(`/admin/gallery/${galleryImage.id}`, data, {
                preserveScroll: true,
            });
        }
    };
                                ...
                                {imagePreview && (
                                    <div className="mt-4">
                                        ...
                                        <img
                                            src={imagePreview}
                                            alt="Preview"
                                            className="max-w-md h-48 object-cover rounded-lg mt-2"
                                        />
                                    </div>
                                )}
